...
{ 
  ...
  cv::Mat imageNew(1,bitstreamBuffer->fillLength,CV_8U);
  
  ROS_INFO_STREAM("debug: 1");
  memcpy(imageNew.data,bitstreamBuffer->bufAddr,bitstreamBuffer->fillLength);
  ROS_INFO_STREAM("debug: 1");
...
{
  int fillLength = bitstreamBuf->fillLength;

  void *bufAddr = bitstreamBuf->bufAddr;
