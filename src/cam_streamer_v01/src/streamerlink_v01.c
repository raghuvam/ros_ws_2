...
char gChains_testMenu[] = {
    ...
    "\r\n ========="
    "\r\n "
    "\r\n Enter 'r' : 1CH VIP capture -> STREAMER LINK -> ROS publisher node (framerStreamHandler)"
    "\r\n "
    "\r\n "
    "\r\n p: CPU Status"
...
int frameStreamHandler(System_BitstreamBuffer *vbitstreamBuffer,NullLink_Obj *vObj)
{
    StreamerLink_BitstreamBuffer* bitstreamBuffer;
    StreamerLink_Obj* pObj;

    //passing params from system_BitsteamBuffer to StreamerLink_bitstreamBuffer
    bitstreamBuffer->bufAddr = vbitstreamBuffer->bufAddr;
    bitstreamBuffer->bufSize = vbitstreamBuffer->bufSize;
    bitstreamBuffer->fillLength = vbitstreamBuffer->fillLength;
    bitstreamBuffer->metaBufAddr = vbitstreamBuffer->metaBufAddr;
    bitstreamBuffer->metaBufSize = vbitstreamBuffer->metaBufSize;
    bitstreamBuffer->metaFillLength = vbitstreamBuffer->metaFillLength;
    bitstreamBuffer->flags = vbitstreamBuffer->flags;
    bitstreamBuffer->width = vbitstreamBuffer->width;
    bitstreamBuffer->height = vbitstreamBuffer->height;

    //passing required params from Nulllink_Obj to frameStreamer_Obj
    pObj->tskId  = vObj->tskId;
    pObj->recvCount   = vObj->recvCount;
    pObj->fpDataStream   = vObj->fpDataStream;

    framePublisher(bitstreamBuffer,pObj);
